...
- **会话窗口健康 + 保鲜**：腾讯只接受「开着会话窗口」时的主动推送，而本地登录 `phase` **反映不出**这一点。网关把每条入站消息落盘成 `last-inbound.json`，把**每次真实发送**的结果落盘成 `window-state.json`，暴露 `GET /window`，`/send` 失败时返回机器可读的 `reason`（`window_closed` / `window_open_invalid_arguments`）与人类可读 `hint`。宿主插件内置保鲜循环：真实发送证明窗口已关时弹**一次**桌面通知；静默超过 `keepaliveWarnHours` 时每个入站再提醒一次。**故意不做探测**——发往不存在收件人永远返回 `ret=-3`，判断不了窗口。**不需要任何本机脚本或 launchd 定时器。**
...
- insert:
    - id: wechat-bot
      ...
      config:
        ...
        keepaliveWarnHours: 2     # 静默达 N 小时就弹桌面提醒（每个入站一次；0=关）
...
| 回复/主动推送失败 | 先查窗口，别只查登录：`curl http://127.0.0.1:51235/window`。`ret=-2 prepare failed` / `reason=window_closed` = 腾讯侧会话窗口已关，光靠入站 `context_token` 打不开（**不带 token 的真实发送同样失败**），让用户给机器人发一句话即可恢复；保鲜循环也会在「开→关」跳变时弹一次桌面通知。若 `window` 为 open 仍失败，才是登录态失效：面板解绑后重新扫码，或删 `~/.dsh-wechat/accounts/` |
| 显示 `window_closed` 但用户说刚发过消息 | 时钟来自真实入站（`last-inbound.json`）：确认手机确实送达、网关日志有该入站行；**不要信 `POST /probe`**——发往不存在收件人在任何窗口状态下都返回 `ret=-3` |
...
| 提交 | 类型 | 内容 |
|---|---|---|
| `0.2.2` | **修复** | **修正窗口判据**（0.2.1 里的探针做不到这件事）：发往不存在收件人在任何窗口状态下都返回 `ret=-3`，从来不代表「窗口开着」——实测探针说 open、一秒钟后**不带 context_token** 的真实发送照样 `ret=-2`。`POST /probe` 改为「只验证上游可达、不作数」且**不再改写 `window-state.json`**；保鲜循环的窗口状态只取**真实发送**的结果，并新增**按静默时长的桌面提醒**（`keepaliveWarnHours`，默认 2 小时，每个入站一次），因为窗口可能在我们观测到之前就关了。同时修掉跨 tick 漏出的旧 action 字段。 |
| `0.2.1` | **修复** | **把「会话窗口健康 + 保鲜提醒」收进插件**（「机器人不通知」的真正根因）：网关对每条入站消息落盘 `last-inbound.json`，暴露 `GET /window`，`/send` 失败时返回机器可读的 `reason`/`hint`；宿主插件内置保鲜循环。存活探针补上 `version` 字段。全新机器安装插件即自带，无需本机脚本/launchd。 |
